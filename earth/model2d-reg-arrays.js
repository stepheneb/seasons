...
model2d.Model2D = function(options) {
    ...
    // temperature array
    this.t = new Array(model2d.ARRAY_SIZE);

    // internal temperature boundary array
    this.tb = new Array(model2d.ARRAY_SIZE);
    ...
    this.u = new Array(model2d.ARRAY_SIZE);
    model2d.fillArray(this.u, 0);
    
    // velocity y-component array (m/s)
    this.v = new Array(model2d.ARRAY_SIZE);
    model2d.fillArray(this.v, 0);

    // internal heat generation array
    this.q = new Array(model2d.ARRAY_SIZE);
    model2d.fillArray(this.q, 0);
    
    
    // wind speed
    this.uWind = new Array(model2d.ARRAY_SIZE);
    model2d.fillArray(this.uWind, 0);
    
    this.vWind = new Array(model2d.ARRAY_SIZE);
    model2d.fillArray(this.vWind, 0);
    
    // conductivity array
    this.conductivity = new Array(model2d.ARRAY_SIZE);
    for (i = 0; i < model2d.ARRAY_SIZE; i++) {
        this.conductivity[i] = this.backgroundConductivity;
    }
    
    // specific heat capacity array
    this.capacity = new Array(model2d.ARRAY_SIZE);
    for (i = 0; i < model2d.ARRAY_SIZE; i++) {
        this.capacity[i] = 0;
    }
    
    // density array
    this.density = new Array(model2d.ARRAY_SIZE);
    for (i = 0; i < model2d.ARRAY_SIZE; i++) {
        this.density[i] = this.backgroundDensity;
    }
    
    // fluid cell array
    this.fluidity = new Array(model2d.ARRAY_SIZE);
    model2d.fillArray(this.fluidity, 0);
...
model2d.fillArray = function(array, fill) {
...
model2d.HeatSolver2D = function(nx, ny, q) {
    ...
    this.t0 = new Array(model2d.ARRAY_SIZE);
    model2d.fillArray(this.t0, 0);
...
model2d.DirichletHeatBoundary = function() {
    this.temperatureAtBorder = new Array(4); // unit: centigrade
...
model2d.FluidSolver2D = function(nx, ny) {
    ...
    this.u0 = new Array(model2d.ARRAY_SIZE);
    model2d.fillArray(this.u0, 0);
    
    this.v0 = new Array(model2d.ARRAY_SIZE);
    model2d.fillArray(this.v0, 0);

    this.vorticity = new Array(model2d.ARRAY_SIZE);
    model2d.fillArray(this.vorticity, 0);
    
    this.stream = new Array(model2d.ARRAY_SIZE);
    model2d.fillArray(this.stream, 0);
};
